...
$fecha_de = '';
$fecha_a = '';
$where = "1"; // Siempre existe una condición válida por defecto
$buscar = '';

// Validación por número de factura
if (!empty($_REQUEST['busqueda'])) {
    if (!is_numeric($_REQUEST['busqueda'])) {
        header("location: ventas.php");
        exit;
    }
    $busqueda = intval($_REQUEST['busqueda']);
...
<section id="container">
    ...
    <table>
        ...
        if ($result > 0) {
            while ($data = mysqli_fetch_array($query)) {
                $estado = ($data["estatus"] == 1) ?
                    '<span class="pagada">Pagada</span>' :
                ...
                <tr id="row_<?php echo $data["nofactura"]; ?>">
                    ...
                    <td class="textrght totalfactura"><span>S/.</span><?php echo $data["totalfactura"]; ?></td>
                    <td>
    ...
    <div class="paginador">
        <ul>
            <?php if ($pagina != 1) { ?>
                <li><a href="?pagina=1&<?php echo $buscar; ?>">|<</a></li>
                <li><a href="?pagina=<?php echo $pagina - 1; ?>&<?php echo $buscar; ?>"><<</a></li>
            <?php }
            for ($i = 1; $i <= $total_paginas; $i++) {
                echo ($i == $pagina) ?
                    '<li class="pageSelected">' . $i . '</li>' :
                    '<li><a href="?pagina=' . $i . '&' . $buscar . '">' . $i . '</a></li>';
            }
